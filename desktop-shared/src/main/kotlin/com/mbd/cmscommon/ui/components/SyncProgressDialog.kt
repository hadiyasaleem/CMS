...
/**
 * Shown for the duration of a manual refresh (mobile: [com.mbd.cmscommon.ui.state.GlobalRefreshViewModel],
 * desktop: each shell's own `refreshShell`/`refreshCurrentScreen`) -- deliberately not dismissible
 * (no back press, no tap-outside) since a refresh left running in the background would let the
 * user navigate away mid-sync and see partially-updated data.
 */
@Composable
