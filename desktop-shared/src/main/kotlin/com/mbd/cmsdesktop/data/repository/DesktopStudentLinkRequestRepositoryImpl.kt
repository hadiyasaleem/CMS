...
/**
 * No local persistence. [cache] holds the full `student_link_requests` table; [sync] replaces it
 * wholesale (a single global table, no scoping args needed — mirroring [DesktopDepartmentRepositoryImpl]).
 * [submitRequest] appends the freshly inserted row directly rather than re-syncing the whole table.
 */
@Singleton
class DesktopStudentLinkRequestRepositoryImpl @Inject constructor(
    ...
    override suspend fun sync() {
        val rows = postgrest.from(SupabaseTables.STUDENT_LINK_REQUESTS).select().decodeList<StudentLinkRequestDto>()
    ...
    override suspend fun submitRequest(
        ...
        cache.value = cache.value + DesktopStudentLinkRequestMapper.dtoToDomain(inserted)
    }
