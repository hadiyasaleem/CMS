...
class InsightsRepositoryImpl @Inject constructor(
    ...
    override suspend fun getSessionOverviews(): List<SessionOverview> =
        postgrest.from(SupabaseTables.VIEW_SESSION_OVERVIEW).select().decodeList<SessionOverviewDto>().map {
            SessionOverview(
    ...
    override suspend fun getAtRiskStudents(): List<AtRiskStudent> =
        postgrest.from(SupabaseTables.VIEW_AT_RISK_STUDENTS).select().decodeList<AtRiskStudentDto>().map {
            AtRiskStudent(
                sessionId = it.sessionId ?: "",
    ...
    override suspend fun getExamStats(): List<ExamStat> =
        postgrest.from(SupabaseTables.VIEW_EXAM_STATS).select().decodeList<ExamStatDto>().map {
            ExamStat(
                sessionId = it.sessionId ?: "",
