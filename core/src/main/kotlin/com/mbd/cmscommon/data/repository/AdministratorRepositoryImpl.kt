...
class AdministratorRepositoryImpl @Inject constructor(
    ...
    override suspend fun sync() {
        ...
        administrators.value = rows.map { it.toDomain() }.sortedBy { it.email }
    }
    ...
    private fun AdministratorAccountDto.toDomain() = AdministratorAccount(
        id = id.orEmpty(),
        email = email.orEmpty(),
        status = status.orEmpty(),
        createdAt = PgTime.parse(createdAt),
        lastLoginAt = PgTime.parse(lastLoginAt),
    )
}
